...
Validates proteon's numerical outputs against widely-used reference
...
### SASA (proteon vs FreeSASA)

**Publication-grade agreement.** Proteon's total SASA matches FreeSASA to
...
Key to matching:
- Use `freesasa.Classifier.getStandardClassifier("protor")` for proteon
  `radii="protor"` parity.
- Set `freesasa.Structure(..., options={"hetatm": True})` because
  proteon counts crystal waters in SASA by default and FreeSASA skips
...
- Use `freesasa.Parameters({"algorithm": freesasa.ShrakeRupley, "n-points": 960})`
  because FreeSASA defaults to Lee-Richards while proteon uses Shrake-Rupley.

Full report: `sota_v1_sasa_report.md`

### Energy (proteon vs OpenMM AMBER96)
...
therefore not noise. Proteon's nonbonded is systematically larger in
magnitude than OpenMM's, with proteon tending toward repulsive totals
while OpenMM gives attractive totals. Proteon matches the BALL AMBER96
...
the same force field, not a proteon FF bug per se.

Candidate causes, in rough order of likelihood:
1. **1-4 interaction scaling**: AMBER usually uses 5/6 for electrostatic
   and 1/2 for vdW. If proteon and OpenMM pick different scale factors
   or apply them to different atom pairs, the nonbonded term diverges.
2. **Terminal residue protonation**: proteon consistently has 1 more
...
The SASA harness smoke-testing against **1bpi** caught a real proteon
...
This creates `/globalscratch/dateschn/proteon-benchmark/sota_venv`, builds
proteon into it via `maturin develop --release`, and installs FreeSASA +
OpenMM + pyarrow + numpy. Idempotent: safe to re-run.

The v1 reference PDBs live at
`/globalscratch/dateschn/proteon-benchmark/sota_pdbs/` — assembled from
the proteon repo `test-pdbs/` (1crn, 1ubq, 1bpi, 1ake) and the monster3
50K corpus (1pgb, 1aki).

## Run (monster3)

```bash
source /globalscratch/dateschn/proteon-benchmark/sota_venv/bin/activate
cd /globalscratch/dateschn/proteon-benchmark/proteon

# SASA comparison
python validation/sota_comparison/run_all.py \
  --pdb-dir /globalscratch/dateschn/proteon-benchmark/sota_pdbs \
  --ops sasa \
...
- **Investigate proteon vs OpenMM AMBER96 nonbonded disagreement**. The
  biggest open question from v1. Start by dumping proteon and OpenMM
  per-atom charges side by side, then compare 1-4 scaling on known bond
  topologies. Reference: BALL oracle test passes proteon to 0.02% on
...
- **Native-binary SOTA tools**: build mkdssp ≥ 4.0 and Reduce from source
  into `/globalscratch/dateschn/proteon-benchmark/sota_bin/`. Wire

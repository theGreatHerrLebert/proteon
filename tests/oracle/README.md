...
| [Molly.jl](https://github.com/JuliaMolSim/Molly.jl) (Julia) | `julia --project=tests/oracle/julia/molly -e 'using Pkg; Pkg.instantiate()'` (pinned by the committed `Manifest.toml`) | `test_molly_energy.py` asserts against frozen values and needs **no** Julia; regenerate them with `molly_energy_oracle.jl` or `validation/amber96_molly_triangulate.py` |

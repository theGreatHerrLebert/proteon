...
class TestDistanceMatrix:
    def test_self_distance_shape(self):
        ...
        dm = ferritin.distance_matrix(coords)
        assert dm.shape == (3, 3)
    ...
    def test_cross_distance_shape(self):
        ...
        dm = ferritin.distance_matrix(a, b)
        assert dm.shape == (5, 7)
...
class TestContactMap:
    ...
    def test_on_structure(self):
        ...
        cm12 = ferritin.contact_map(ca, cutoff=12.0)
    ...
    def test_cross_contacts(self):
        ...
        cm = ferritin.contact_map(a, cutoff=2.0, coords2=b)
        assert cm.shape == (1, 2)
        assert cm[0, 0] == True
        assert cm[0, 1] == False
...
class TestDihedralAngle:
    def test_cis(self):
        ...
        d = ferritin.dihedral_angle(p0, p1, p2, p3)
    ...
    def test_range(self):
        ...
        for _ in range(100):
            pts = rng.randn(4, 3)
            d = ferritin.dihedral_angle(pts[0], pts[1], pts[2], pts[3])
...
class TestGeometry:
    ...
    def test_rg_on_structure(self):
        s = load_crambin()
        rg = ferritin.radius_of_gyration(s.coords)
...
class TestToDataframe:
    ...
    def test_invalid_engine(self):
        ...
        with pytest.raises(ValueError, match="Unknown engine"):
            ferritin.to_dataframe(s, engine="spark")
...
class TestIntegration:
    def test_contact_map_decreases_with_cutoff(self):
        s = load_crambin()
        ca = ferritin.extract_ca_coords(s)
        c6 = ferritin.contact_map(ca, cutoff=6.0).sum()
        c8 = ferritin.contact_map(ca, cutoff=8.0).sum()
        c12 = ferritin.contact_map(ca, cutoff=12.0).sum()
        assert c6 <= c8 <= c12
    ...
    def test_cross_distance_between_structures(self):
        s1 = load_crambin()
        s2 = load_ubiquitin()
        ca1 = ferritin.extract_ca_coords(s1)
        ca2 = ferritin.extract_ca_coords(s2)
        dm = ferritin.distance_matrix(ca1, ca2)

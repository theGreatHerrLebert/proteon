...
) -> proteon_align::search::alphabet::AlphabetResult {
...
fn build_result_dict<'py>(
    py: Python<'py>,
    records: &[ResidueBackboneRecord],
    result: proteon_align::search::alphabet::AlphabetResult,
        ...
        pool.install(|| {
            target_alphabets
                ...
                .map(|(target_alphabet, target_aa)| {
                    ungapped_diagonal_score(&query_alphabet, &query_aa, target_alphabet, target_aa)
                })
                .collect::<Vec<_>>()
        })
    });

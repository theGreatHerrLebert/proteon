//! Ferritin Data Engine — bulk structure → Parquet pipeline.
//!
//! Reads PDB/mmCIF files, writes per-atom Arrow/Parquet tables.
//!
//! Usage:
//!     ferritin-ingest input/ --out features.parquet
//!     ferritin-ingest *.pdb --out features.parquet
//!     ferritin-ingest input/ --out output/ --per-structure
...
use ferritin_arrow::convert::pdb_to_atom_batch;

#[derive(Parser)]
#[command(
    name = "ferritin-ingest",
...
fn main() -> Result<()> {
    ...
    eprintln!(
        "ferritin-ingest: {} structures, {} threads",
        files.len(),
    ...
    if args.per_structure {
        ...
        let schema = ferritin_arrow::atom::atom_schema();
...
fn load_and_write_single(
    ...
    ferritin_arrow::writer::write_parquet(&out_path, &schema, &[batch])?;
    Ok(total_atoms)
}

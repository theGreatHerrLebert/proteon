...
try:
    ...
    _search = proteon_connector.py_search
...
def _require_search_backend():
    if _search is None:
        raise ImportError(
            "proteon search encoding requires the native proteon-connector "
            "extension. Install `proteon` from PyPI or build "
            "`proteon-connector` with `maturin develop --release`."
        )
    return _search
...
def _candidate_foldseek_3di_matrix_paths() -> List[Path]:
    ...
    env_path = os.environ.get("PROTEON_FOLDSEEK_3DI_MATRIX")
    if env_path:
...
def encode_alphabet(structure) -> Dict[str, Any]:
    """Encode a protein structure into proteon's 20-state structural alphabet.

...
def _warn_amber96_cutoff_policy() -> None:
    ...
    crambin with both tools at NoCutoff, proteon-AMBER96 agrees with
    OpenMM-AMBER96 to **0.2%** on total energy — bond 0.017%, angle
    0.22%, torsion 0.44%, nonbonded 0.26%. The force-field math is
    reference-quality.

    Proteon's *default* production setting uses a 15 Å nonbonded cutoff
    ...
    """
    warnings.warn(
        "proteon's AMBER96 force-field math matches OpenMM AMBER96 to "
        "0.2% on crambin when both use NoCutoff. The default proteon "
...
def compute_energy(
    ...
    Force field status:
        ...
        * ``"amber96"`` — **oracle-validated** against OpenMM AMBER96 on
          ...
          NoCutoff. The default proteon setting applies a 15 Å
...
def gpu_available() -> bool:
    ...
    Examples:
        >>> proteon.gpu_available()
        True
        >>> if proteon.gpu_available():

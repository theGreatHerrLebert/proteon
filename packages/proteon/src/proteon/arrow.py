...
pandas, polars, DuckDB, Spark, and Parquet files.
    ...
    >>> ipc = proteon.to_arrow(s, "1crn")      # → bytes (Arrow IPC)
    >>> proteon.to_parquet(s, "out.parquet")     # → direct Parquet file
    >>>
...
try:
    ...
    _arrow = proteon_connector.py_arrow
...
def _check_available():
    if _arrow is None:
        raise ImportError(
            "proteon Arrow support requires the proteon-connector native extension. "
            "Rebuild with: cd proteon-connector && maturin develop --release"
        )
...
def from_arrow(data: bytes) -> List[Tuple[str, Structure]]:
    ...
    Agent Notes:
        WATCH: Expects proteon's per-atom schema, not an arbitrary Arrow table.
...
def from_parquet(path: str) -> List[Tuple[str, Structure]]:
    ...
    Agent Notes:
        WATCH: Expects a proteon atom-schema Parquet file, not a generic Parquet table.

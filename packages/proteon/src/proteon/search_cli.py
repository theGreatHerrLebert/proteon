...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Matches the library default. Note the effective rerank depth is max(top_k,
    # rerank_top_k), so at top_k=10 this already TM-aligns 10 candidates. Raising it
    # deepens the reranked pool (a measured recall bump in one benchmark run; see
    # SEARCH_ROADMAP.md §quality) at proportionally more TM-align work.
    p_query.add_argument("--rerank-top-k", type=_positive_int, default=5, help="How many top hits to TM-align rerank (effective depth is max(top-k, this); default 5).")

...
use proteon_electrostatics::{
    ...
    solve_local_elements_treecode, solve_surface, Charge, FastSummation, LinearOperator,
    LocalOperator, Locality, Params, SolveConfig, SurfaceSolveOptions, Tri, TWO_PI,
};
...
fn treecode_local_solve_matches_dense_and_born() {
    ...
    assert!(
        rel(e_dense, born) < 0.03 && rel(e_tree, born) < 0.03,
        "should track Born"
    );
...
fn treecode_solution_solves_the_dense_system() {
    ...
    assert!(
        rel_resid < 2e-3,
        "treecode u doesn't solve the dense system: {rel_resid:.3e}"
    );
}
...
fn solve_surface_fast_summation_matches_dense() {
    ...
    let dense_opts = SurfaceSolveOptions {
        params,
        cfg,
        ..Default::default()
    };
    let out_dense = solve_surface(mesh.clone(), &charges, &dense_opts)
        .result
        .expect("dense");
    ...
    let out_tc = solve_surface(mesh, &charges, &tc_opts)
        .result
        .expect("treecode");
    ...
    let diff: Vec<f64> = out_tc
        ...
        .collect();
    assert!(
        l2(&diff) / dp.max(1e-300) < 5e-3,
        "per-vertex potential drift too large"
    );
}
...
fn treecode_solve_rejects_bad_params_without_panicking() {
    ...
    assert!(
        solve_local_elements_treecode(&els, &charges, &params, &cfg, 0, 0.5).is_err(),
        "p=0"
    );
    assert!(
        solve_local_elements_treecode(&els, &charges, &params, &cfg, 9999, 0.5).is_err(),
        "p too large"
    );
    assert!(
        solve_local_elements_treecode(&els, &charges, &params, &cfg, 6, 1.0).is_err(),
        "theta≥1"
    );
    assert!(
        solve_local_elements_treecode(&els, &charges, &params, &cfg, 6, 0.0).is_err(),
        "theta=0"
    );
}

#[test]
fn fast_summation_nonlocal_routes_to_treecode() {
    // fast_summation now drives the NONLOCAL treecode solve too (P8.4). On a tiny mesh
    // this just checks the routing succeeds + converges; correctness vs dense is gated in
    // tests/p8_nonlocal_solve.rs.
    let mesh = analytic_sphere_mesh(2.0, 1);
    let charges = [Charge {
        pos: Vec3::new(0.0, 0.0, 0.0),
        val: 1.0,
    }];
    ...
    let opts = SurfaceSolveOptions {
        ...
        cfg,
        nonlocal: true,
        fast_summation: Some(FastSummation { p: 6, theta: 0.5 }),
        ..Default::default()
    };
    let out = solve_surface(mesh, &charges, &opts);
    let sol = out.result.expect("nonlocal treecode solve should succeed");
    assert!(sol.converged);
    assert!(sol.rfenergy < 0.0, "solvation energy is negative");
}

#[test]
fn fast_summation_nonlocal_invalid_params_rejected() {
    // The treecode params are validated for the nonlocal path now that it uses them.
    let mesh = analytic_sphere_mesh(2.0, 1);
    let charges = [Charge {
        pos: Vec3::new(0.0, 0.0, 0.0),
        val: 1.0,
    ...
    assert!(
        solve_surface(mesh, &charges, &opts).result.is_err(),
        "theta=5 must be rejected"
    );
}
...
fn treecode_accuracy_tightens_with_p() {
    ...
    let err_at = |p: usize| {
        let (rt, _) = solve_local_elements_treecode(&els, &charges, &params, &cfg, p, 0.5).unwrap();

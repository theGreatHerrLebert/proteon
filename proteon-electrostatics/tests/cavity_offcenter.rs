...
fn concentric_kirkwood_reductions() {
    ...
    for &s in &[0.0_f64, 0.5, 1.0] {
        ...
        assert!(
            (multi - single).abs() / single.abs() < 1e-12,
            "s={s}: {multi} vs {single}"
        );
    }
    ...
    let l0 = concentric_shell_rfenergy(1.0, &[(1.0, EO, ES), (2.0, ES, EO), (3.0, EO, ES)]);
    assert!(
        (multi0 - l0).abs() / l0.abs() < 1e-12,
        "offset 0: {multi0} vs {l0}"
    );
}
...
fn offcenter_cavity_bem_matches_kirkwood() {
    ...
    for &offset in &[0.0_f64, 0.4, 0.7] {
        ...
        assert!(
            rel < 0.04,
            "offset {offset}: off Kirkwood by {rel:.3} (> 4%)"
        );
    }
}
